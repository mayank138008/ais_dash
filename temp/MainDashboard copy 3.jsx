...
// // import { useRef } from 'react'; // ✅ Make sure this is at the top with other imports
...
export default function Dashboard() {
  ...
  // useEffect(() => {
  //   console.log('🔥 activeFeature changed to:', activeFeature);
  //   console.log('🖼️ overlay:', overlaySources[activeFeature]);

  //   async function loadStats() {
  //     try {
  //       if (activeFeature && overlaySources[activeFeature]) {
  //         console.log('🧪 Fetching stats for overlay:', overlaySources[activeFeature]);    
...
useEffect(() => {
  async function loadStats() {
    console.log('🌀 useEffect triggered for activeFeature:', activeFeature);

    if (!activeFeature || !overlaySources[activeFeature]) {
      console.log('🚫 No activeFeature or overlay source');
      setStats({});
      return;
    }

    // Only skip if previousFeature is the same AND stats already exist
    if (
      previousFeature.current === activeFeature &&
      stats.Feature === activeFeature
    ) {
      console.log('🔁 Skipping redundant fetch for:', activeFeature);
      return;
    }
    ...
    try {
      ...
      if (!result) {
        console.warn('⚠️ getImageStatsFromOverlay returned null/falsy');
        setStats({});
        return;
      }

      console.log('📊 New stats:', result);
